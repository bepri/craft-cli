...
from craft_cli._rs.printer import Printer
...
class _PipeReaderThread(threading.Thread):
    ...
    def _write(self, data: bytes) -> None:
        ...
        while True:
            ...
            text = f":: {unicode_line}"
            self.printer.show(
                self.stream.fileno() if self.stream else None, text, **self.printer_flags
            )
...
class _StreamContextManager:
    ...
    def __init__(
        ...
        if text is not None:
            ...
            printer.show(stream.fileno() if stream else None, text, **printer_flags)
...
class _Handler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        self.printer.show(
            stream.fileno() if stream else None,
            text,
            use_timestamp=use_timestamp,
            ephemeral=ephemeral,
        )
...
class Emitter:
    ...
    def message(self, text: str) -> None:
        ...
        """
        stream = None if self._mode == EmitterMode.QUIET else sys.stdout.fileno()
    ...
    def trace(self, text: str) -> None:
        ...
        if self._mode == EmitterMode.TRACE:
            self._printer.show(sys.stderr.fileno(), text, use_timestamp=True)
    ...
    def _report_error(self, error: errors.CraftError) -> None:
        ...
        use_timestamp = True
        exception_stream = sys.stderr.fileno()
        if self._mode in (EmitterMode.QUIET, EmitterMode.BRIEF, EmitterMode.VERBOSE):
            use_timestamp = False
        ...
        for line in str(error).splitlines():
            self._printer.show(
                sys.stderr.fileno(), line, use_timestamp=use_timestamp, end_line=True
            )
        ...
        if isinstance(error, errors.CraftCommandError):
            ...
            if stderr:
                ...
                self._printer.show(
                    sys.stderr.fileno(), text, use_timestamp=use_timestamp, end_line=True
                )

        # detailed information and/or original exception
        if error.details:
            text = f"Detailed information: {error.details}"
            details_stream = None if self._mode == EmitterMode.QUIET else sys.stderr.fileno()
